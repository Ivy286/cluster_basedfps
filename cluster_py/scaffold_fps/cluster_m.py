...
class ChemParse(object):
    ...
    # 读文件或dataframe，返回mol对象和ids
    def input_reader(self, smi_col='Smiles', id_col='ID'):
        # print('*******'*10)
        csv_mols = []
        ids_new = []
        for smi_item, id_item in zip(self.input_df[smi_col], self.input_df[id_col]):
            csv_mol = Chem.MolFromSmiles(smi_item)
            if csv_mol:
    ...
    def clusterOutput(self, output, cdict, index_=0):
        ...
        if output[-4:] == '.sdf':
            ...
            for m, n in zip(self.source, self.ids):
                ...
                m.SetProp("fp_cluster_label", str(classid))
        ...
        if output[-4:] == '.csv':
            ...
            ids_ = []
            for m, n in zip(self.source, self.ids):
                class_id.append(index_ + int(cdict[index][0]))
                centroid_id.append(cdict[index][1])
                smiles.append(Chem.MolToSmiles(m))
                ids_.append(n)
                index += 1
            out_df = pd.DataFrame({'Smiles': smiles, 'ID': ids_, 'fp_cluster_label': class_id, 'isCentroid': centroid_id})
