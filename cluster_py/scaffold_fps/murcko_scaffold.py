...
def cluster_by_Murcko_scaffold(smiles_list, id_list, use_generics=True, remove_linker_side_chain=True):
    ...
    for smi_item, id_item in zip(smiles_list, id_list):
        ...
        if count % 500 == 0:
            print('finished scaffold clustering molecules %d / %d' % (count, total_count))
...
def smis_dataprocess(read_text, smi_col=0, id_col=1):
    print('======== Based on scaffold cluster starting ========')
    # if input_file[-4:] == '.csv':
    ...
    df_clustered['scaffold_cluster_label'] = clustered_label_list

    count_list = get_cluster_count(scaffold2smiles_dict, clustered_scaffold_list, use_str=True)

    print('*' * 50)
    print('Length of all molecules: ', len(count_list))
    df_clustered['num_count'] = count_list
    print('Total scaffold cluster number:', len(scaffold_set))
    df_clustered = pd.merge(df_clustered, read_text[['drug_id', 'Energy']],
                            left_on='ID', right_on='drug_id').drop(['drug_id'], axis=1)
    df_clustered.to_csv('./based_scaffold_cluster.csv', index=False)
    print('=' * 60)
    return df_clustered
