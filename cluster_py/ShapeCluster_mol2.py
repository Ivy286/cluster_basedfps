# Cluster based on molecule 3D shape
...
def do_voxels(mol2_list, name_list):
    ...
    def shape_cluster(num_clusters):
        y_pred = KMeans(n_clusters=int(num_clusters), random_state=42).fit_predict(voxels_array)
        # print(y_pred.shape)
    ...
    param = {
        'num_clusters': ('int', [3, 50]),
    }

    random_state = 42
    gp_max_iter = 10
    np.random.seed(random_state)
    gpgo = GPGO(gp, acq, shape_cluster, param)
    gpgo.run(max_iter=gp_max_iter, init_evals=2)
    para_result = gpgo.getResult()[0]['n_clusters']
    print('Shape cluster: {}'.format(para_result))
    print('*'*60)

    kmeans_model = KMeans(n_clusters=int(para_result), random_state=random_state).fit(voxels_array)
    return kmeans_model.labels_


def main():
    mol2_path = '151.mol2'
    output_csv = 'shape_cluster_output.csv'
    output_sdf = 'shape_cluster_output.sdf'
    ...
    # write csv file
    smis_list = [Chem.MolToSmiles(Chem.MolFromMol2Block(i, sanitize=True)) for i in mol2s
                 if Chem.MolFromMol2Block(i, sanitize=True)]
    pd.DataFrame({'Smiles': smis_list, 'ID': new_names,
                  'shape_class': label_.tolist()}).to_csv(output_csv, index=False)
